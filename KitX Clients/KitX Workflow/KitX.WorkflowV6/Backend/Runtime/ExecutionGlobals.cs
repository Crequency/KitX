...
public class ExecutionGlobals
{
    ...
    // ── Dict function family ──
    // Dict is KScript's first-class mutable key-value container (Dictionary<string, object?>),
    // distinct from JSON (read-only JsonElement). Dict construction happens via the
    // dict-literal declaration initialiser (const/var block); these functions operate on
    // already-constructed Dict values. DictSetValue/DictRemove/DictMerge mutate in place
    // and return the same reference. See Package/Dict-Type-Design.md.
