...
///
/// The runtime already hands plugin returns to the workflow as a <see cref="JsonElement"/>
/// (boxed as object, via RealPluginManager.ParseResult&lt;object&gt;), but the type system never
/// admitted it. These helpers normalize "anything JSON-shaped" (already a JsonElement, a JSON
/// string, or a serializable object) into a JsonElement so the JSON builtin family can consume
/// plugin returns and string literals uniformly.
/// </summary>
public static class JsonElementExtensions
{
    /// <summary>Coerce a value (object?) into a JsonElement. Accepts:
    /// <list type="bullet">
    ///   <item>An existing <see cref="JsonElement"/> (returned as-is, possibly cloned if it
    ///       originated from a disposable JsonDocument).</item>
