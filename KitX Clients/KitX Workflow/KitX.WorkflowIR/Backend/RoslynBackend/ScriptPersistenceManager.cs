...
// ─────────────────────────────────────────────────────────────────────────────
// ScriptPersistenceManager — direct port of the legacy
// KitX.Workflow.Compilation.ScriptPersistenceManager. Persists compiled workflow
// assemblies to disk ({hash}.dll + {hash}.meta.json) under
// Data/CompiledScripts/{workflow-id}/, with version-based invalidation.
//
// Only the namespace changed; the load/save/preload/delete semantics are identical.
// ─────────────────────────────────────────────────────────────────────────────
...
{
    /// <summary>Root directory for persisted compiled assemblies (Data/CompiledScripts/).</summary>
    internal static readonly string CompiledScriptsRoot = Path.Combine("./Data/", "CompiledScripts");

    /// <summary>JSON serializer options for meta.json persistence.</summary>
    ...
    /// <summary>Attempts to load a compiled script from disk (null if absent/stale/corrupt).</summary>
    internal ICompiledBlockScript? TryLoadFromDisk(string workflowId, string hash)
    {
        ...
        try
        {
            ...
            if (scriptType == null)
            {
                Log.Debug("[ScriptPersistenceManager] Type '{Type}' not found in disk assembly for hash '{Hash}'", meta.TypeName, hash);
                alc.Unload();
                DeleteFromDisk(workflowId, hash);
                return null;
            }
            ...
            _registerCacheEntry(hash, new CompiledScriptEntry(instance, alc));
            return instance;
        }
        catch (Exception ex)
        {
