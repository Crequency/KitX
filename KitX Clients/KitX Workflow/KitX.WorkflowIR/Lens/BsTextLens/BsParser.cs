...
//      IReadOnlyList, but passing List<> satisfies that implicitly.
//   3. diagnostics → DiagnosticSink (wrapping List<LoweringDiagnostic>) instead
//      of the legacy ConversionDiagnostics bag.
//
// BlockDefinition is now an immutable record. The legacy ParseBlock mutated
// block.Statements in place (Append + comment back-fill). To keep the migrated
// logic readable, ParseBlock now accumulates into local mutable lists and
// constructs the immutable BlockDefinition once at the end.
//
// The 28 EBNF productions, the self-describing builtin dispatch
// (IBuiltinFunction + IParserHandler.ParseInvocation), and the v5.0/v5.1
// diagnostic codes (BS_ILLEGAL_ASSIGNMENT, BS_NESTED_CALL, BS_RESERVED_PLACEHOLDER, …)
...
public static class BsParser
{
    ...
    /// converter carries as a string). Returns null when the string is empty or not a
    /// single parseable expression.
    /// </summary>
    ...
    /// <summary>
    /// Parse the body of a recognized block into a <see cref="BlockDefinition"/>.
    ...
    {
        var name = string.IsNullOrEmpty(recognized.BlockName)
            ? recognized.BlockType.ToString()
            : recognized.BlockName;

        // BlockDefinition is an immutable record now; accumulate into locals and build once.
        List<VariableDeclaration> blockVars = new();
        List<VariableDeclaration> variables = new();
        List<BlockStatement> statements = new();

        // BlockVars sub-section (##BlockVars) — declarations only.
        if (!string.IsNullOrEmpty(recognized.BlockVarsContent))
            blockVars = ParseDeclarations(recognized.BlockVarsContent, recognized, diagnostics);
        ...
        if (recognized.BlockType is BlockType.ConstBlock or BlockType.PubVarBlock)
        {
            if (!string.IsNullOrEmpty(recognized.Content))
                variables = ParseDeclarations(recognized.Content, recognized, diagnostics);
            return BuildBlock(recognized, name, variables, blockVars, statements);
        }

        // MainBlock / NamedBlock — statements.
        if (!string.IsNullOrEmpty(recognized.Content))
            ParseStatements(recognized, registry, diagnostics, statements);

        // v5.1: extract comments from source and attach to statements.
        ExtractComments(recognized.Content, statements, recognized.StartLine, out var finalStatements);

        return BuildBlock(recognized, name, variables, blockVars, finalStatements);
    }

    static BlockDefinition BuildBlock(RecognizedBlock recognized, string name,
        List<VariableDeclaration> variables, List<VariableDeclaration> blockVars,
        List<BlockStatement> statements) => new()
        {
    ...
    /// </summary>
    static void ExtractComments(string content, List<BlockStatement> statements, int blockStartLine,
        out List<BlockStatement> finalStatements)
    {
        finalStatements = statements;
        if (string.IsNullOrEmpty(content) || statements.Count == 0) return;
        ...
        // Statements are immutable records; rebuild the list applying comments via `with`.
        var applied = statements.ToList();
        if (standaloneComments.Count > 0)
        {
            int si = 0; // statement index
            foreach (var (_, text) in standaloneComments)
            {
                if (si < applied.Count)
                    applied[si] = applied[si] with { Comment = text };
                si++;
            }
        }
        ...
        foreach (var (absLine, text) in inlineComments)
        {
            for (int i = 0; i < applied.Count; i++)
            {
                ...
                {
                    applied[i] = stmt with { Comment = text };
                    break;
                }
            }
        }

        finalStatements = applied;
    }

    // ─── Helpers ────────────────────────────────────────────────────────────

    static List<VariableDeclaration> ParseDeclarations(
        string text, RecognizedBlock recognized, DiagnosticSink diagnostics)
    {
        ...
        List<BlockStatement> statements)
    {
        ...
        List<BSExpression> parsedStatements;
        try
        {
            parsedStatements = StatementList.Parse(tokens);
        }
        catch (ParseException ex)
        {
        ...
        foreach (var parsed in parsedStatements)
            DispatchStatement(parsed, recognized, registry, diagnostics, statements);
    }

    // v5.1: BuildFlowControlFromArgLayout / ExtractStringArg removed.
    // Each flow-control function parses its own invocation via
    // IParserHandler.ParseInvocation (self-describing).

    static void DispatchStatement(
        BSExpression parsed, RecognizedBlock recognized,
        BuiltinFunctionRegistry? registry, DiagnosticSink diagnostics,
        List<BlockStatement> statements)
    {
        ...
        // dispatched via IParserHandler.ParseInvocation, or a side-effect call (Print).
        if (parsed is BSCall call)
        {
            ...
            // The registry exposes per-role handlers; GetParser returns the IParserHandler.
            var parserHandler = registry?.GetParser(call.MethodName);
            if (parserHandler is not null && parserHandler.ParseInvocation(call, line) is { } flowStmt)
            {
                statements.Add(flowStmt);
                return;
            }
