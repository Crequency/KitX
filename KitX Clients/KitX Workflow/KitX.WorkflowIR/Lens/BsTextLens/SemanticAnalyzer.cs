...
//
// L147 fix (CollectForLoopIndexNames): the legacy code matched a FlowControlStatement
// purely on FunctionName == "ForLoop", which would misfire if any other function ever
// reused that name. The fix keys the match off the registry's declared behavioural
// kind + the concrete ForLoopFunction type, so only a genuine ForLoop (ControlFlow
// kind, ForLoopFunction descriptor) contributes its indexName. This makes V3
// (ForLoop-index read-only) robust against name collisions.
// ─────────────────────────────────────────────────────────────────────────────

/// <summary>
/// v5.0 semantic validation pass. Runs after parsing (Layer 1) and before IR lowering.
/// All v5.0 hard constraints that go beyond syntax are checked here,
/// producing <see cref="DiagnosticSink"/> error/warning entries.
/// </summary>
internal sealed class SemanticAnalyzer
{
    ...
    public SemanticAnalyzer(BuiltinFunctionRegistry? registry, DiagnosticSink diagnostics)
    {
        _registry = registry;
    ...
    /// Call after <see cref="BsParser.ParseBlock"/> and before IR lowering.
    /// </summary>
    ...
    private void ValidateConstWrite(BSExpression expr, IReadOnlyList<VariableDeclaration> constVars,
    ...
    private HashSet<string> CollectForLoopIndexNames(List<BlockDefinition> blocks)
    {
        ...
        foreach (var block in blocks)
        {
            foreach (var stmt in block.Statements)
            {
                // L147 fix: the legacy match was a bare FunctionName == "ForLoop" string
                // compare. That is fragile — any function reusing the name would trip V3.
                // Key the match off the registry's declared behavioural kind AND the
                // concrete descriptor type so only a genuine ForLoop contributes its
                // indexName. When no registry is available we fall back to the name match
                // (preserving legacy behaviour for registry-less parse paths).
                if (stmt is FlowControlStatement fc && fc.FlowArguments.Count >= 4)
                {
                    bool isForLoop;
                    if (_registry is not null)
                    {
                        var def = fc.FunctionName is null ? null : _registry.Get(fc.FunctionName);
                        isForLoop = def?.Kind == FunctionKind.ControlFlow && def is ForLoopFunction;
                    }
                    else
                    {
                        isForLoop = fc.FunctionName == "ForLoop";
                    }

                    if (isForLoop)
                    {
                        // FlowArguments[3] is the indexName (already stripped of quotes by ParseInvocation).
