...
static class TestComments
{
        ...
        Action<string, string> pass, Action<string, string, string> fail)
    {
        ...
        if (shouldRun("T51")) { var src = "#ConstBlock\nint a = 3;\n\n#MainBlock\na > Print;\nGoto(\"LoopBody\");\n\n// This is a block-level comment for LoopBody\n#Block LoopBody\na > Print;\nGoto(\"End\");" + End; var rt = roundTrip(parser, converter, reverseConverter, src, h); bool ok = rt != null && rt.Contains("// This is a block-level comment"); if (ok) pass("T51", "block-level comment retained"); else fail("T51", "block-level comment retained", "Block-level comment not retained (§9.3)"); }
