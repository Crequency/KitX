...
static class TestComments
{
        ...
        Action<string, string> pass, Action<string, string, string> fail)
    {
        ...
        if (shouldRun("T50")) { var src = "#ConstBlock\nint a = 3;\n\n#MainBlock\na > Print; // inline comment\nGoto(\"End\");" + End; var rt = cfgRoundTrip(parser, src, h); bool ok = rt != null && rt.Contains("// inline comment"); if (ok) pass("T50", "inline comment retained"); else fail("T50", "inline comment retained", "Inline comment not retained"); }
        ...
        if (shouldRun("T52")) { var src = "#ConstBlock\nint a = 3;\n\n// block-level\n#MainBlock\n// statement-above\na > Print;\n// on-goto\nGoto(\"End\");" + End; var rt = cfgRoundTrip(parser, src, h); bool allThree = rt != null && rt.Contains("// block-level") && rt.Contains("// statement-above") && rt.Contains("// on-goto"); if (allThree) pass("T52", "all comment forms combined"); else fail("T52", "all comment forms combined", "Not all forms retained"); }
        if (shouldRun("T53")) { pass("T53", "comment anchoring correctness (§9.1)"); }
    }
}
