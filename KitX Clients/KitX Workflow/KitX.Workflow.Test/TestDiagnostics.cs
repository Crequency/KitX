...
static class TestDiagnostics
{
        ...
        Action<string, string, bool, string> check)
    {
        ...
        if (shouldRun("T54")) { var pr = parser.Parse("#MainBlock\nPrint(Get(\"x\"));\nGoto(\"End\");" + End); var diag = pr.Diagnostics; check("T54", "nested call → BS_NESTED_CALL error", diag != null && diag.Items.Any(d => d.Code == "BS_NESTED_CALL"), ""); }
        ...
        if (shouldRun("T57")) { var pr = parser.Parse("#ConstBlock\nint x = 5;\n\n#MainBlock\n0 > x;\nGoto(\"End\");" + End); var diag = pr.Diagnostics; if (diag != null && diag.HasErrors) pass("T57", "ConstBlock write → error"); else fail("T57", "ConstBlock write → error", "ConstBlock read-only enforcement not implemented (§3.1)"); }
        if (shouldRun("T58")) { var pr = parser.Parse("#ConstBlock\nint loopMax = 3;\n\n#MainBlock\nGoto(\"LoopHead\");\n\n#Block LoopHead\nForLoop(0, loopMax, 1, \"i\", \"Body\", \"End\");\n\n#Block Body\n5 > i;\nGoto(\"LoopHead\");" + End); var diag = pr.Diagnostics; if (diag != null && diag.HasErrors) pass("T58", "ForLoop index write → error"); else fail("T58", "ForLoop index write → error", "ForLoop index read-only enforcement not implemented (§7.1)"); }
